...
//                    next = neighbors[i];
                ...
                if (constraint_flow[curr->curr_timestep + curr->get_g() + 1][next].first || // vertex conflict
                    constraint_flow[curr->curr_timestep + curr->get_g()][curr->id].second[d] || // edge conflict
                    constraint_flow[curr->curr_timestep + curr->get_g()][next].second[rev_d])
                    op_flow = (env->cols + env->rows) * 2;
                else
                    op_flow = 0;
