...
//a astar minimized the opposide traffic flow with existing traffic flow
    ...
    s_node backward_astar(SharedEnvironment *env, std::vector<Int4> &flow, Traj &traj, FlowHeuristic& fh,
        ...
        init_traffic_heuristic(fh, env, goal, start);

        s_node *root = mem.get_node(goal);
        ...
        s_node *goal_node = root;
        s_node *start_node;
        int neighbors[4];


        while (!fh.open.empty()) {
            s_node *curr = fh.open.pop();
            assert(fh.htable[curr->id] == curr->g);
            curr->close();
            ...
            for (int i = 0; i < 4; i++) {
                ...
                temp_op = ((flow[next].d[d] + 1) * flow[curr->id].d[(d + 2) % 4]);
                ...
                if (!mem.has_node(next)) {
                    s_node *next_node = mem.generate_node(next, cost, h, 0, 0, 0);
                    next_node->parent = curr;
                    fh.open.push(next_node);
                    fh.htable[next] = cost;
                }
                else {
                    s_node *existing = mem.get_node(next);
                    assert(existing->g == fh.htable[next]);
        ...
        traj.clear();
        s_node *curr = start_node;

        while (curr->parent != nullptr)
        {
            traj.push_back(curr->id);
            curr = curr->parent;
        }
        traj.push_back(goal_node->id);
